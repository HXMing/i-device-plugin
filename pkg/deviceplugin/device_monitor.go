...
func NewDeviceMonitor(devicepath string) *DeviceMonitor {
...
func (d *DeviceMonitor) Watch() error {
	klog.Info("watching devices")

	w, err := fsnotify.NewWatcher()
	if err != nil {
		return errors.WithMessage(err, "new watch failed")
	}
	defer w.Close()
	...
	err = w.Add(d.path)
	if err != nil {
		klog.Infof("watch device error: %v", err)
	}
	return <-errChan
}
